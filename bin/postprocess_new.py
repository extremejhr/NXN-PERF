...
class CaseResult(object):
    ...
    def module_time_extract(self):
        ...
        if len(self.name_list) > 0:
            ...
            for file in self.name_list:
                ...
                f04raw = open(gl.get_value('RESULT_PATH') + file + '.f04')

                line_stack = f04raw.readlines()

                elapsed_time, date = self.others_time(line_stack)

                line_process = self.line_preprocess(line_stack)

                submodule_list, list_com = self.module_line_extract(line_process)

                module_time = self.mainmodule_time(line_process, list_com)
                ...
                for sub in submodule_list:
                    ...
                    if sub[0] not in list_dict.keys():
                        ...
                        list_dict[sub[0]] = self.module_seperate(line_process, range(sub[1] + 1, sub[2]), 1)

                    else:

                        list_dict[sub[0]] = list_dict[sub[0]] + self.module_seperate(line_process,
                ...
                for key in list_dict.keys():
                    ...
                    if len(list_dict[key]) > 0:
                        ...
                        submodule_time = self.submodule_time(line_process, list_dict[key])

                        results[key] = submodule_time

                whole_results.append(results)

        return whole_results

    def line_preprocess(self, line_stack):
    ...
    def module_seperate(self, line_process, span_list, flag):
    ...
    def module_line_extract(self, line_process):

        submodule_list = []

        module_list = []

        list_com = self.module_seperate(line_process, range(len(line_process)), 0)
        ...
        list_com1 = self.module_seperate(line_process, list(whole_set - submodule_set), 1)

        return submodule_list, list_com1

    def mainmodule_time(self, line, list_dict_com):
    ...
    def submodule_time(self, line, list_dict_com):
